...
class Series extends BaseController
{
    ...
    public function series(int $idSerie)
    {
        ...
        $descriptionShift = $shift =='M'?'MANHA':'TARDE';        
        $nameDir = $description.'-'. $clasfication.'-'. $descriptionShift;
        $nameArchive = $nameDir.'.pdf';
            ...
            for ($dw = 2; $dw < 7; $dw++) {
                ...
                foreach ($result as $item) {
                    if ($item->position == $ps && $item->dayWeek == $dw) {
                        ...
                        $dataSchedule = mb_convert_encoding($item->description,$this->encode)."\n".mb_convert_encoding(abbreviationTeacher($item->name),$this->encode);
