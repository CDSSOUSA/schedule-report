...
class SchedulePDF extends \FPDF
{
    ...
    public function Header()
    {
        ...
        $this->Image(getenv('URL.FRONTEND') . "assets/img/{$this->logoInstituicao}", 15, 10, 45); // importa uma imagem
        $this->SetXY(65, 5);
        $this->SetMargins(20, 20, 40, 20);
        $this->MultiCell(0, 4, mb_convert_encoding($textoCabecalho, $this->encode), 0, "L");
        $this->Ln();
    }
    function Footer()
    {
        ...
        $formatDate = new IntlDateFormatter('pt-br', IntlDateFormatter :: LONG, IntlDateFormatter:: NONE);
        //$textoCidade = 'Campina Grande,' . (strftime('%d de %B de %Y', strtotime(date('Y-m-d')))) . '.';
        $textoCidade = 'Campina Grande, ' . $formatDate->format(time());
        $this->MultiCell(0, 6, mb_convert_encoding($textoCidade, $this->encode), 0, 'C');
        /* FIM */
        ...
        // Page number
        $this->Cell(0, 5, mb_convert_encoding('Página ', $this->encode) . $this->PageNo() . '/{nb}', 0, 0, 'C');
    }
